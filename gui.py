...
def codes_submit():
    ...
    #调用main库中测试函数。
    main.test_codes(codes_data, test_data, time_limit_temp, ram_limit_temp,
                    space_value)
...
label_title = tkinter.Label(root,
                            text='OJ在线评测系统',
                            font=('Microsoft YaHei Light', 25))
#允许撤销操作。加粗边框。
text_codes = tkinter.Text(root,
                          width=45,
                          height=22,
                          borderwidth=10,
                          font=('Microsoft YaHei Light', 15),
                          undo=True)
label_codes = tkinter.Label(root,
                            text='代码框',
                            font=('Microsoft YaHei Light', 15))
#允许撤销操作。加粗边框。
text_test = tkinter.Text(root,
                         width=41,
                         height=10,
                         borderwidth=10,
                         font=('Microsoft YaHei Light', 15),
                         undo=True)
label_test = tkinter.Label(root,
                           text='测试框',
                           font=('Microsoft YaHei Light', 15))
label_time = tkinter.Label(root,
                           text='时间限制(ms):',
                           font=('Microsoft YaHei Light', 15))
label_ram = tkinter.Label(root,
                          text='内存限制(mb):',
                          font=('Microsoft YaHei Light', 15))
label_space = tkinter.Label(root,
                            text='是否严格限制空格与换行',
                            font=('Microsoft YaHei Light', 15))
...
button_submit = tkinter.Button(root,
                               ...
                               font=('Microsoft YaHei Light', 15),
                               width=10,
                               command=codes_submit)
button_exit = tkinter.Button(root,
                             text='退出',
                             font=('Microsoft YaHei Light', 15),
                             width=10,
                             command=window_destroy)
button_space_yes = tkinter.Button(root,
                                  text='是',
                                  font=('Microsoft YaHei Light', 15),
                                  width=10,
                                  command=space_yes)
button_space_no = tkinter.Button(root,
                                 text='否',
                                 font=('Microsoft YaHei Light', 15),
                                 width=10,
                                 command=space_no)
