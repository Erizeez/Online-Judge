...
def test_codes(codes_data, test_data, time_limit_temp, ram_limit_temp,
               space_value):
    ...
    for i in range(len(data_modified[0])):
        #创建CodeTest实例。
        testing = codes_class.CodeTest(data_modified[0][i],
                                       data_modified[1][i], space_value,
                                       time_limit_value, ram_limit_value)
    ...
    for j in range(len(result)):
        if result[j] != 'TLE':
            temp = result[j] + ' on test' + '%s' % (j + 1) + ' | time:' + str(
                time[j] * 1000) + 'ms' + ' | memory used:' + str(
                    memory[j]) + 'mb' + '\n'
            message += temp
        else:
            temp = result[j] + ' on test' + '%s' % (j + 1) + ' | time:' + str(
                time_limit_value) + 'ms' + ' | memory used:' + str(
                    memory[j]) + 'mb' + '\n'
            message += temp
    ...
    #布置反馈信息和返回按钮。
    label_message = tkinter.Label(call_back,
                                  text='%s' % message,
                                  font=('Microsoft YaHei Light', 15))
    button_back = tkinter.Button(call_back,
                                 text='返回',
                                 font=('Microsoft YaHei Light', 15),
                                 width=10,
                                 command=close_info)
    ...
    #和其他情况下红色字体显示。
    if result_symbol == 'AC':
        label_final_result = tkinter.Label(call_back,
                                           text='%s' % result_symbol,
                                           fg='green',
                                           font=('Microsoft YaHei Light', 50))
    else:
        label_final_result = tkinter.Label(call_back,
                                           text='%s' % result_symbol,
                                           fg='red',
